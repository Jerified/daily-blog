...
const RecentPost = async ({ data }: any) => {
    ...
    return (
        <section className="max-w-5xl mx-auto pt-16 w-full">
                    ...
                    {data.map((post: any) => {
                        return (
                            <div className="flex gap-3 h-full w-full " key={post._id}>
                                <Image className=' w-[15rem] h-[15vh] object-cover rounded-lg overflow-hidden fle' src={post.mainImage} alt={post.alt} width={100} height={100} objectFit='cover' />
                                <div className='text-start flex flex-col flex-wrap items-start'  >
                                    <Link href={`/blog/${post.slug.current}`} className="text-md md:text-lg font-bold">{post.title}</Link>
                                    <p className="text-xs tracking-wide sm:text-[0.8rem] dark:text-gray-300">{post.excerpt}</p>
